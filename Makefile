...
.DEFAULT_GOAL := help
.PHONY: userlocation greet blog calculator help
project := userlocation greet calculator blog

all: $(project) ## Generate Pbs and build

userlocation: $@ ## Generate Pbs and build for userlocation
...
clean: clean_userlocation clean_greet clean_calculator clean_blog ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_userlocation: ## Clean generated files for userlocation
	${RM_F_CMD} userlocation/${PROTO_DIR}/*.pb.go
